...
import { FiUsers, FiCheckCircle, FiChevronRight, FiSettings } from 'react-icons/fi';
...
const AdminDashboard = () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:1010/users/${selectedUser._id}/approve`,
        {
          ...
          organizationalUnits: selectedUser.organizationalUnits.map(ou => ou.value),
  ...
  const customStyles = {
    ...
    option: (provided, state) => ({
      ...provided,
      backgroundColor: state.isSelected ? '#4299e1' : state.isFocused ? '#4a5568' : '#2d3748',
      color: 'white'
    }),
    multiValue: (provided) => ({
      ...provided,
      ...
      <div className="container-fluid">
        <div className="row mb-4">
          <div className="col">
            ...
            <p className="text-gray-400">Approve new users and assign access privileges</p>
          </div>
        </div>
  ...
  >
    ...
    <div 
      ...
      style={{
        ...
        maxHeight: 'calc(100vh - 250px)' // Limit height
      }}
    >
      {isLoading ? (
        ...
        <div className="list-group list-group-flush">
          {users.map((user, index) => (
            ...
            >
              <button
                className={`list-group-item list-group-item-action d-flex justify-content-between align-items-center ${selectedUser?._id === user._id ? 'active' : ''}`}
                style={{
                  backgroundColor: selectedUser?._id === user._id ? '#4299e1' : '#2d3748',
                  borderColor: '#4a5568',
                  color: 'white',
                  whiteSpace: 'nowrap',
                  overflow: 'hidden',
              ...
              >
                <div style={{ minWidth: 0 }}>
                  <h6 className="mb-1 text-truncate">{user.fullName}</h6>
  ...
  {selectedUser ? (
    <motion.div
      ...
      style={{ 
        backgroundColor: '#2d3748',
        maxHeight: 'calc(100vh - 200px)', // Limit card height
    ...
    >
      <div className="card-header bg-dark">
        <h5 className="m-0 text-white">Assign Access for {selectedUser.fullName}</h5>
      </div>
      
      {/* Scrollable content area */}
      <div 
        className="card-body overflow-y-auto" 
        style={{ flex: '1 1 auto' }} // Takes available space
      >
       <div className="mb-4">
  <label className="form-label text-white">User Role</label>
  <Select
    options={roleOptions}
    value={roleOptions.find(opt => opt.value === selectedUser.roles)}
    onChange={(selected) => setSelectedUser({
      ...selectedUser,
  ...
  />
        ...
        <div className="mb-4">
          <label className="form-label text-white">Organizational Units</label>
          <Select
            ...
            menuPlacement="auto" // Smart menu positioning
          />
        </div>
    ...
    >
      <div className="card-body d-flex flex-column justify-content-center">
        ...
        <p className="text-gray-200">
          Please select a user from the list to assign access privileges
        </p>
      </div>
    </motion.div>
  )}
